                ...
                <div class="card-body">
                    <form action="{{ route('gallery.update', $item->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select name="travel_packages_id" class="form-control">
                                <option value="{{$item->travel_packages_id}}">Click to Update Title {{$item->travel_package->title}}</option>
                                    @foreach ($travel_packages as $travel_package)
                                        <option value="{{$travel_package->id}}">{{$travel_package->title}}</option>
                                    @endforeach
                            </select>
                        ...
                        <div class="float-left">
                            <a class="btn btn-secondary px-4" href="{{route('gallery.index')}}">Kembali</a>
                            <button type="submit" class="btn btn-primary" value="{{$item->image}}">Update</button>
                        </div>
                    </form>
                </div>
            </div>
